...
//! - On Wayland the window is not dropped. minifb 0.28's Wayland backend destroys its event
//!   queue before the objects on it, and libwayland prints a warning listing them, so the window
//!   is left for process exit to release, which closes the connection and lets the compositor
//!   free them. On X11 it drops as usual.
...
/// Open the window, run the loop, and on Wayland leave the window for process exit to release.
///
/// The window is forgotten however the loop ended, an error included, since the warning comes
/// from its drop, see the module doc.
fn main() -> Result<(), BoxError> {
    ...
    if on_wayland(&window) {
        mem::forget(window);
    }
    result
}

/// Loop until Return or a close: redraw on a size change, present, and poll.
///
...
///   pass only polls.
fn run(window: &mut Window) -> Result<(), BoxError> {
...
/// Whether minifb opened the window with its Wayland backend, the one whose drop warns.
///
/// Asks the window's own handle rather than the environment, since minifb tries Wayland first and
/// falls back to X11.
