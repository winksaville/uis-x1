...
/// The denominator a scale factor is read over, Wayland's unit for fractional scales.
const SCALE_DENOMINATOR: u32 = 120;

/// The scale for a window's scale factor: the factor as a ratio over 120, Wayland's own unit.
///
/// A factor that is not a positive number becomes 0, which draws as 1 once rounded.
pub fn scale_for(scale_factor: f64) -> Ratio {
    let num = (scale_factor * f64::from(SCALE_DENOMINATOR)).round() as u32;
    Ratio::new(num, SCALE_DENOMINATOR).unwrap_or(Ratio::ONE) // OK: the denominator is 120
}
...
mod tests {
    ...
    /// A scale factor becomes its exact ratio over 120, reduced.
    #[test]
    fn scale_is_the_factor_as_a_ratio() {
        let ratio = |num, den| Ratio::new(num, den).expect("nonzero denominator");
        assert_eq!(scale_for(1.0), Ratio::ONE);
        assert_eq!(scale_for(1.25), ratio(5, 4));
        assert_eq!(scale_for(1.5), ratio(3, 2));
        assert_eq!(scale_for(2.0), Ratio::from(2));
        assert_eq!(scale_for(1.0 / 3.0), ratio(1, 3));
        assert_eq!(scale_for(f64::NAN), Ratio::from(0));
    }
}
